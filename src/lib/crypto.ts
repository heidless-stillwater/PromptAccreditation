...
/**
 * Decrypts a string encrypted with the standard Stillwater AES-256-GCM format.
 * Format: v2:iv:authTag:encryptedContent
 */
export function decrypt(encryptedText: string): string {
    const masterKey = process.env.CONFIG_ENCRYPTION_KEY;
    if (!masterKey || masterKey.length < 64) {
        throw new Error('CONFIG_ENCRYPTION_KEY missing or invalid in environment');
    }

    if (!encryptedText.startsWith('v2:')) {
        throw new Error('Unsupported encryption format. Expected prefix "v2:"');
    }
